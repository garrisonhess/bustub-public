...
TEST_F(RecoveryTest, DISABLED_RedoTest) {
  ...
  LogManager::Get(*db).RunFlushThread();
  ASSERT_TRUE(enable_logging);
  LOG_INFO("System logging thread running...");

  LOG_INFO("Create a test table");
  Transaction *txn = TransactionManager::Get(*db).Begin();
  auto *test_table = new TableHeap(*db, txn);
  ...
  TransactionManager::Get(*db).Commit(txn);
  LOG_INFO("Commit txn");
