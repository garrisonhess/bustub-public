...
unique_ptr<SampleOptions> Transformer::TransformSampleOptions(bustub_libpgquery::PGNode *options) {
	if (!options) {
		return nullptr;
	}
	auto result = make_unique<SampleOptions>();
	auto &sample_options = (bustub_libpgquery::PGSampleOptions &)*options;
	auto &sample_size = (bustub_libpgquery::PGSampleSize &)*sample_options.sample_size;
