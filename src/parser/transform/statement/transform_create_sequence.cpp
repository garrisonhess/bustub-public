...
#include "bustub/parser/parsed_data/create_sequence_info.hpp"
#include "bustub/parser/transformer.hpp"
#include "bustub/common/operator/cast_operators.hpp"

namespace bustub {

unique_ptr<CreateStatement> Transformer::TransformCreateSequence(bustub_libpgquery::PGNode *node) {
	auto stmt = reinterpret_cast<bustub_libpgquery::PGCreateSeqStmt *>(node);
	...
	if (stmt->options) {
		...
		for_each_cell(cell, stmt->options->head) {
			...
			if (def_elem->defaction == bustub_libpgquery::PG_DEFELEM_UNSPEC && !val) { // e.g. NO MINVALUE
				continue;
			}
			D_ASSERT(val);
			int64_t opt_value;
			if (val->type == bustub_libpgquery::T_PGInteger) {
				opt_value = val->val.ival;
			} else if (val->type == bustub_libpgquery::T_PGFloat) {
