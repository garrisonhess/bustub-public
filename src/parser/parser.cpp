	...
	if (!statements_.empty()) {
		...
		for (auto &statement : statements_) {
			...
			if (statement->type_ == StatementType::CREATE_STATEMENT) {
				...
				create.info->sql = query.substr(statement->stmt_location_, statement->stmt_length_);
			}
		}
	}
}
...
bool Parser::IsKeyword(const std::string &text) {
...
std::vector<std::unique_ptr<ParsedExpression>> Parser::ParseExpressionList(const std::string &select_list, ParserOptions options) {
...
std::vector<OrderByNode> Parser::ParseOrderList(const std::string &select_list, ParserOptions options) {
	...
	parser.ParseQuery(mock_query);
	// check the statements
	if (parser.statements_.size() != 1 || parser.statements_[0]->type != StatementType::SELECT_STATEMENT) {
		throw Exception("Expected a single SELECT statement");
	}
	auto &select = (SelectStatement &)*parser.statements_[0];
	if (select.node->type != QueryNodeType::SELECT_NODE) {
...
std::vector<std::vector<std::unique_ptr<ParsedExpression>>> Parser::ParseValuesList(const std::string &value_list, ParserOptions options) {
	// construct a mock query
