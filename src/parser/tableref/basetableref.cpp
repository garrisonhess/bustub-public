...
string BaseTableRef::ToString() const {
  string schema = schema_name_.empty() ? "" : KeywordHelper::WriteOptionallyQuoted(schema_name_) + ".";
  string result = schema + KeywordHelper::WriteOptionallyQuoted(table_name_);
...
bool BaseTableRef::Equals(const TableRef *other_p) const {
  ...
  return other->schema_name_ == schema_name_ && other->table_name_ == table_name_ &&
         column_name_alias_ == other->column_name_alias_;
}
