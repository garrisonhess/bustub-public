...
// 	uint64_t end_pos = query.size();
// 	for (uint64_t i = error_location; i < query.size(); i++) {
// 		if (StringUtil::CharacterIsNewline(query[i])) {
// 			end_pos = i;
// 			break;
// 		}
...
// 		for (uint64_t cpos = 0; cpos < len; cpos++) {
// 			positions.push_back(cpos);
...
// 	uint64_t current_render_width = 0;
// 	for (uint64_t i = epos; i > 0; i--) {
...
// 	for (uint64_t i = epos; i < positions.size(); i++) {
// 		current_render_width += render_widths[i];
// 		if (current_render_width >= 40) {
// 			truncate_end = true;
// 			end_pos = positions[i];
...
// 	uint64_t error_render_width = 0;
// 	for (uint64_t i = spos; i < epos; i++) {
