...
ClientContext::ClientContext(BusTub &database)
    : db_(database),
      transaction_(*database.transaction_manager_),
      interrupted_(false),
      executor_(*this),
      catalog_(*database.catalog_),
      temporary_objects_(make_unique<SchemaCatalogEntry>(db_.catalog_.get(), TEMP_SCHEMA, true)),
      prepared_statements_(make_unique<CatalogSet>(*db_.catalog_)),
...
unique_ptr<DataChunk> ClientContext::FetchInternal() { return executor.FetchChunk(); }
...
void ClientContext::Interrupt() { interrupted = true; }
