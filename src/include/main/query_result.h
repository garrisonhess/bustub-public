...
//   QueryResult(QueryResultType type, StatementType statement_type, vector<Type> types, vector<string> names);
//   //! Creates an unsuccessful query result with error condition
//   QueryResult(QueryResultType type, string error);
//   virtual ~QueryResult() = default;

//   //! The type of the result (MATERIALIZED or STREAMING)
//   QueryResultType type_;
...
//   unique_ptr<QueryResult> next_;

//  public:
//   //   //! Fetches a DataChunk from the query result. Returns an empty chunk if the result is empty, or nullptr on
//   //   failure. virtual unique_ptr<DataChunk> Fetch() = 0;
...
//   uint64_t ColumnCount() { return types_.size(); }
