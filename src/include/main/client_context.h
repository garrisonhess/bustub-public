...
class ClientContext {
 public:
  explicit ClientContext(BusTub &database);
  ...
  Catalog &catalog_;
  unique_ptr<SchemaCatalogEntry> temporary_objects_;
  unique_ptr<CatalogSet> prepared_statements_;

  // Whether or not aggressive query verification is enabled
  bool query_verification_enabled_ = false;
  //! Enable the running of optimizers
  bool enable_optimizer_ = true;
  //! Force parallelism of small tables, used for testing
  bool force_parallelism_ = false;
  //! Force index join independent of table cardinality, used for testing
  bool force_index_join_ = false;
  //! Maximum bits allowed for using a perfect hash table (i.e. the perfect HT can hold up to 2^perfect_ht_threshold
  //! elements)
  uint64_t perfect_ht_threshold_ = 12;
  //! The writer used to log queries (if logging is enabled)
  unique_ptr<BufferedFileWriter> log_query_writer_;
  // //! The explain output type used when none is specified (default: PHYSICAL_ONLY)
  // ExplainOutputType explain_output_type_ = ExplainOutputType::PHYSICAL_ONLY;
 ...
 public:
  ...
  //   unique_ptr<PreparedStatementData> CreatePreparedStatement(const string &query, unique_ptr<SQLStatement>
  //   statement);
