...
HavingBinder::HavingBinder(Binder &binder, ClientContext &context, BoundSelectNode &node, BoundGroupInformation &info,
    ...
    : SelectBinder(binder, context, node, info), column_alias_binder_(node, alias_map) {
...
BindResult HavingBinder::BindColumnRef(unique_ptr<ParsedExpression> *expr_ptr, uint64_t depth, bool root_expression) {
  ...
  auto alias_result = column_alias_binder_.BindAlias(*this, expr, depth, root_expression);
  if (!alias_result.HasError()) {
    return alias_result;
  }

  return BindResult(StringUtil::Format(
      "column %s must appear in the GROUP BY clause or be used in an aggregate function", expr.ToString().c_str()));
}
...
BindResult HavingBinder::BindExpression(unique_ptr<ParsedExpression> *expr_ptr, uint64_t depth, bool root_expression) {
  ...
  switch (expr.expression_class_) {
    ...
    default:
      return bustub::SelectBinder::BindExpression(expr_ptr, depth, false);
  }
}
