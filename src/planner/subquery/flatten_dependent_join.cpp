...
#include "planner/subquery/has_correlated_expressions.h"
#include "planner/subquery/rewrite_correlated_expressions.h"
  ...
  switch (plan->type) {
    ...
    case LogicalOperatorType::LOGICAL_AGGREGATE_AND_GROUP_BY: {
      ...
      if (aggr.groups.size() == correlated_columns.size()) {
        ...
        for (uint64_t i = 0; i < correlated_columns.size(); i++) {
          JoinCondition cond;
          cond.left = make_unique<BoundColumnRefExpression>(correlated_columns[i].type, ColumnBinding(left_index, i));
    ...
    case LogicalOperatorType::LOGICAL_COMPARISON_JOIN: {
      ...
      for (uint64_t i = 0; i < correlated_columns.size(); i++) {
        ...
        if (join.type == LogicalOperatorType::LOGICAL_COMPARISON_JOIN) {
          ...
          auto conjunction = make_unique<BoundConjunctionExpression>(ExpressionType::CONJUNCTION_AND, move(comparison),
                                                                     move(any_join.condition));
    ...
    default:
      throw Exception("Logical operator type \"%s\" for dependent join", LogicalOperatorToString(plan->type));
  }
}
