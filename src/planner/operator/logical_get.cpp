...
LogicalGet::LogicalGet(uint64_t table_index, TableFunction function, unique_ptr<FunctionData> bind_data,
    ...
    : LogicalOperator(LogicalOperatorType::LOGICAL_GET),
      ...
      returned_types(move(returned_types)),
      names(move(returned_names)) {}

string LogicalGet::GetName() const { return StringUtil::Upper(function.name); }
...
uint64_t LogicalGet::EstimateCardinality(ClientContext &context) {
