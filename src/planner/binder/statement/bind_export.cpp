...
#include "catalog/catalog_entry/copy_function_catalog_entry.h"
...
bool IsExistMainKeyTable(string &table_name, vector<TableCatalogEntry *> &unordered) {
  for (uint64_t i = 0; i < unordered.size(); i++) {
...
void ScanForeignKeyTable(vector<TableCatalogEntry *> &ordered, vector<TableCatalogEntry *> &unordered,
  ...
  for (auto i = unordered.begin(); i != unordered.end();) {
    ...
    for (uint64_t j = 0; j < table_entry->constraints.size(); j++) {
  ...
  for (auto &table : tables) {
    ...
    while (true) {
      ...
      if (table->schema->name == DEFAULT_SCHEMA) {
        info->file_path =
            fs.JoinPath(stmt.info->file_path, StringUtil::Format("%s%s.%s", SanitizeExportIdentifier(table->name),
        ...
        info->file_path = fs.JoinPath(
            ...
            StringUtil::Format("%s_%s%s.%s", SanitizeExportIdentifier(table->schema->name),
                               SanitizeExportIdentifier(table->name), id_suffix, copy_function->function.extension));
      }
    ...
    if (child_operator) {
      ...
      auto copy_union =
          make_unique<LogicalSetOperation>(GenerateTableIndex(), 1, move(child_operator), move(bound_statement.plan),
