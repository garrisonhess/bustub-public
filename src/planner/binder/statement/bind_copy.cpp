...
#include "function/table/read_csv.h"
...
BoundStatement Binder::BindCopyTo(CopyStatement &stmt) {
  ...
  auto function_data = copy_function->function.copy_to_bind(context, *stmt.info, select_node.names, select_node.types);
...
BoundStatement Binder::BindCopyFrom(CopyStatement &stmt) {
  ...
  for (uint64_t i = 0; i < bound_insert.expected_types.size(); i++) {
