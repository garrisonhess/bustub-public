...
BoundStatement Binder::Bind(InsertStatement &stmt) {
  ...
  vector<uint64_t> named_column_map;
  if (!stmt.columns.empty()) {
    // insertion statement specifies column list

    // create a mapping of (list index) -> (column index)
    case_insensitive_map_t<uint64_t> column_name_map;
  ...
  uint64_t expected_columns = stmt.columns.empty() ? table->columns.size() : stmt.columns.size();
  ...
  if (values_list) {
    ...
    for (uint64_t col_idx = 0; col_idx < expected_columns; col_idx++) {
      ...
      for (uint64_t list_idx = 0; list_idx < expr_list.values.size(); list_idx++) {
