...
unique_ptr<BoundTableRef> Binder::Bind(TableFunctionRef &ref) {
  ...
  auto func_catalog = catalog.GetEntry(context, CatalogType::TABLE_FUNCTION_ENTRY, fexpr->schema, fexpr->function_name,
                                       false, error_context);
  ...
  if (func_catalog->type == CatalogType::TABLE_FUNCTION_ENTRY) {
    ...
    uint64_t bind_index = query->GetRootIndex();
    // string alias;
  ...
  uint64_t best_function_idx = Function::BindFunction(function->name, function->functions, arguments, error);
  ...
  for (uint64_t i = 0; i < ref.column_name_alias.size() && i < return_names.size(); i++) {
    return_names[i] = ref.column_name_alias[i];
  }
  for (uint64_t i = 0; i < return_names.size(); i++) {
