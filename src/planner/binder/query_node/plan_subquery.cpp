#include "function/aggregate/distributive_functions.h"
...
#include "planner/subquery/flatten_dependent_join.h"
...
static unique_ptr<Expression> PlanUncorrelatedSubquery(Binder &binder, BoundSubqueryExpression &expr,
  ...
  switch (expr.subquery_type) {
    case SubqueryType::EXISTS: {
      ...
      auto uint64_type = count_star->return_type;
      ...
      auto left_child = make_unique<BoundColumnRefExpression>(uint64_type, ColumnBinding(aggregate_index, 0));
      auto right_child = make_unique<BoundConstantExpression>(Value::Numeric(uint64_type, 1));
      auto comparison =
    ...
    case SubqueryType::SCALAR: {
      ...
      auto first_agg = AggregateFunction::BindAggregateFunction(binder.context, FirstFun::GetFunction(expr.return_type),
    ...
    default: {
      ...
      uint64_t mark_index = binder.GenerateTableIndex();
...
static void CreateDelimJoinConditions(LogicalDelimJoin &delim_join, vector<CorrelatedColumnInfo> &correlated_columns,
                                      vector<ColumnBinding> bindings, uint64_t base_offset) {
  for (uint64_t i = 0; i < correlated_columns.size(); i++) {
    auto &col = correlated_columns[i];
    JoinCondition cond;
...
static unique_ptr<Expression> PlanCorrelatedSubquery(Binder &binder, BoundSubqueryExpression &expr,
  ...
  switch (expr.subquery_type) {
    case SubqueryType::SCALAR: {
      ...
      return make_unique<BoundColumnRefExpression>(expr.GetName(), expr.return_type, plan_columns[flatten.data_offset]);
    }
    case SubqueryType::EXISTS: {
