...
class BoundSubqueryNode : public QueryNode {
 public:
  BoundSubqueryNode(shared_ptr<Binder> subquery_binder, unique_ptr<BoundQueryNode> bound_node,
      ...
      : QueryNode(QueryNodeType::BOUND_SUBQUERY_NODE),
        subquery_binder(move(subquery_binder)),
        bound_node(move(bound_node)),
        subquery(move(subquery)) {}
...
BindResult ExpressionBinder::BindExpression(SubqueryExpression &expr, uint64_t depth) {
  if (expr.subquery->node->type != QueryNodeType::BOUND_SUBQUERY_NODE) {
    ...
    expr.subquery->node = make_unique<BoundSubqueryNode>(move(subquery_binder), move(bound_node), move(prior_subquery));
  }
