...
#include "planner/expression/bound_case_expression.h"
...
static Type ResolveInType(OperatorExpression &op, vector<BoundExpression *> &children) {
  ...
  for (uint64_t i = 1; i < children.size(); i++) {
    max_type = Type::MaxType(max_type, children[i]->expr->return_type);
  }
...
static Type ResolveOperatorType(OperatorExpression &op, vector<BoundExpression *> &children) {
  switch (op.type) {
...
BindResult ExpressionBinder::BindGroupingFunction(OperatorExpression &op, uint64_t depth) {
  return BindResult("GROUPING function is not supported here");
}

BindResult ExpressionBinder::BindExpression(OperatorExpression &op, uint64_t depth) {
  ...
  switch (op.type) {
    ...
    case ExpressionType::STRUCT_EXTRACT: {
      ...
      if (extract_exp.expr->return_type.id() != TypeId::STRUCT &&
          extract_exp.expr->return_type.id() != TypeId::SQLNULL) {
        return BindResult(
  ...
  Type result_type = ResolveOperatorType(op, children);
