...
hash_t BoundAggregateExpression::Hash() const {
  ...
  result = CombineHash(result, bustub::Hash(distinct));
  return result;
}
...
unique_ptr<Expression> BoundAggregateExpression::Copy() {
  ...
  auto copy = make_unique<BoundAggregateExpression>(function, move(new_children), move(new_filter), move(new_bind_info),
                                                    distinct);
