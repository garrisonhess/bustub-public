...
BoundFunctionExpression::BoundFunctionExpression(Type return_type, ScalarFunction bound_function,
    ...
    : Expression(ExpressionType::BOUND_FUNCTION, ExpressionClass::BOUND_FUNCTION, move(return_type)),
      function(move(bound_function)),
      children(move(arguments)),
...
unique_ptr<Expression> BoundFunctionExpression::Copy() {
  ...
  auto copy =
      make_unique<BoundFunctionExpression>(return_type, function, move(new_children), move(new_bind_info), is_operator);
  copy->CopyProperties(*this);
  return move(copy);
}

void BoundFunctionExpression::Verify() const { D_ASSERT(!function.name.empty()); }
